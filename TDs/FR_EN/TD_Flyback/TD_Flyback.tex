...
\ifthenelse{\equal{\Langue}{FR}}{
\item  Représenter sur une période de découpage l'allure des signaux $ i_1$, $i_2$, $v_T$, $v_1$ et $\varphi$.
}{
\item Plot the waveforms of $i_1$, $i_2$, $v_T$, $v_1$ and $\varphi$ over one switching period.
}

\ifthenelse{\equal{\SuCorr}{CO}}{
\begin{figure}[H]
\centering
\includegraphics[width=0.45\linewidth]{TD_Flyback/fig/Flyback_Q22_I12.png}
\includegraphics[width=0.45\linewidth]{TD_Flyback/fig/Flyback_Q22_phi_v12.png}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$i_{1\ \max} = \dfrac{E}{L_1} \alpha_0 T$

\ifthenelse{\equal{\Langue}{FR}}{
Continuité du flux à}{Flux continuity at} $t= \alpha T$ $\rightarrow$ $n_2 i_{2 \ \max} = n_1 i_{1\ \max}$ $\rightarrow$ $i_{2\ \max} = \dfrac{E \alpha_0 T}{L_1 m}$

Hopkinson: $n_1 i_1 + n_2 i_2 = n_1 i_{10}$

$\alpha_0 T < t < \alpha'T$: $i_1=0$ $\rightarrow$ $n_2 i_2 = n_1 i_{10}$ $\rightarrow$ $\dfrac{d i_2}{d_t} = -\dfrac{1}{m} \dfrac{d i_{10}}{dt}= \dfrac{1}{L_1 m} v_1$ 

$\dfrac{d i_2}{dt} = \dfrac{E+\dfrac{V_s}{m}}{L_1 m}$

\ifthenelse{\equal{\Langue}{FR}}{Durée de conduction de D}{Conduction time of D}:
$\Delta t = \dfrac{i_{2\ \max}}{\dfrac{d i_2}{dt}} = \dfrac{E \alpha_0 T}{(E+\dfrac{V_s}{m})} = 11 \mu s$

\ifthenelse{\equal{\Langue}{FR}}{On peut vérifier que $\Delta t < (1 - \alpha_0) T = 25 \mu s$}{We can verify that $\Delta t < (1 - \alpha_0) T = 25 \mu s$} $\rightarrow$ \ifthenelse{\equal{\Langue}{FR}}{Démagnétisation totale}{Full demagnetisation}
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$W_D = E <i_1> T = 0.01 J$ \ifthenelse{\equal{\Langue}{FR}}{(Energie inchangée)}{(Energy unchanged)}

$W_D = \dfrac{V_S I_S}{f}$ $\rightarrow$ $I_S = \dfrac{W_D f}{V_S} = 8.3 A$ 
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$I_s = \dfrac{W_D f}{V_S} = \dfrac{E <i_1>}{V_S} = \dfrac{E}{V_S} \alpha_1 \dfrac{E}{2 L_1} \alpha_1 T = \dfrac{E^2 \alpha_1^2}{2 V_S L_1 f}$

$\rightarrow \alpha_1 = \sqrt{\dfrac{2 V_S L_1 f I_{S1}}{E^2}} = 0.17$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$\varphi_{\max} = \dfrac{E}{n_1} \alpha T$ $\rightarrow$ $B_{\max} = \dfrac{\varphi_{\max}}{A_e} = \dfrac{E \alpha T}{n_1 A_e}$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$A_{e\ \min} = \dfrac{E \alpha T}{n_1 B_{sat}}$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$i_{1 \max} = \dfrac{2 m I_S}{1-\alpha}$

$I_{1\ eff}^2 = \dfrac{1}{T} \int_0^{T} i_1^2 dt = \dfrac{1}{T} \int_0^{\alpha T} {\left(i_{1 \max}\dfrac{t}{\alpha T}\right)}^2 dt = \dfrac{i_{1 \max}^2}{\alpha^2 T^3} \dfrac{{(\alpha T)}^3}{3} = i_{1 \max}^2\dfrac{\alpha}{3}$

$I_{1\ eff} = i_{1 \max} \sqrt{\dfrac{\alpha}{3}} = \dfrac{2 m I_S}{1-\alpha} \sqrt{\dfrac{\alpha}{3}}$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$I_{2\ eff} = i_{2 \max} \sqrt{\dfrac{1-\alpha}{3}} = \dfrac{i_{1 \max}}{m} \sqrt{\dfrac{1-\alpha}{3}} = \dfrac{2 I_S}{1-\alpha} \sqrt{\dfrac{1-\alpha}{3}}$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$S_{T\ Cu} = n_1 \dfrac{I_{1\ eff}}{\delta} + n_2 \dfrac{I_{2\ eff}}{\delta} = \dfrac{2 n_2 I_S}{\delta (1-\alpha)} \sqrt{\dfrac{\alpha}{3}} + \dfrac{2 n_2 I_S}{\delta (1-\alpha)} \sqrt{\dfrac{1-\alpha}{3}}$

$S_{T\ Cu} = \dfrac{2 n_2 I_S}{\delta (1-\alpha)} \left(\sqrt{\dfrac{\alpha}{3}} + \sqrt{\dfrac{1-\alpha}{3}}\right)$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$S_{B \min} = \dfrac{S_{T\ Cu}}{k_B} = \dfrac{2 n_2 I_S}{k_B \delta (1-\alpha)} \left(\sqrt{\dfrac{\alpha}{3}} + \sqrt{\dfrac{1-\alpha}{3}}\right)$
}
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$A_e S_B > \dfrac{E \alpha T}{n_1 B_{sat}} \dfrac{2 n_2 I_S}{k_B \delta (1-\alpha)} \left(\sqrt{\dfrac{\alpha}{3}} + \sqrt{\dfrac{1-\alpha}{3}}\right)$

$P_S = V_S I_S = \dfrac{E m \alpha}{1-\alpha} I_S$ $\rightarrow$ $A_e S_B > \dfrac{2 P_S T}{B_{sat} k_B \delta} \left(\sqrt{\dfrac{\alpha}{3}} + \sqrt{\dfrac{1-\alpha}{3}}\right)$
}
...
\hline
\ifthenelse{\equal{\Langue}{FR}}{\multirow{2}*{Noyau}}{\multirow{2}*{Core}} & \multirow{2}*{$A_e\ (mm^2)$} & \multirow{2}*{$S_B \ (mm^2)$} & $A_L$ \ifthenelse{\equal{\Langue}{FR}}{sans gap d'air}{without air gap} & $A_L$ 0.5 mm \ifthenelse{\equal{\Langue}{FR}}{gap d'air}{air gap}\\
& & & (nH) & (nH)\\
\hline
RM6 & 31 & 32 & 3100 & ---\\
\hline
...
\hline
\ifthenelse{\equal{\Langue}{FR}}{Noyau}{Core} & $A_e\ (mm^2)$ & $S_B \ (mm^2)$ & $A_e S_B \ (mm^4)$\\
\hline
RM6 & 31 & 32 & 992 \\
\hline
RM8 & 55 & 57 & 3135 \\
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$ B < B_{sat} \rightarrow n_1 > \dfrac{E \alpha T}{B_{sat} A_e} = 110.3 \rightarrow n_1 = 111$
...
\ifthenelse{\equal{\SuCorr}{CO}}{
$L_1 = n_1^2 A_L \rightarrow A_L = \dfrac{L_1}{n_1^2} = 227 nH$ 

\ifthenelse{\equal{\Langue}{FR}}{Besoin d'un entrefer supérieur à 0.5 mm}{Need for an air gap greater than 0.5 mm}}
